...
An already-materialized repo is the offline fallback for an ordinary fetch
failure. Attachment keeps the readable checkout, marks that source as using cached
bytes in the Project brief, and continues; a merge failure after a successful fetch
remains strict because the fetched state is not safely reconcilable. A changed
remote is cloned into staging and atomically swapped into the stable source path,
with rollback preserving the old checkout if publication fails. A Project
attachment error is reported as a Project setup failure and does not increment the
