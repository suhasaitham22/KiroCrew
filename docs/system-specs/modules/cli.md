...
| `kirocrew pod up/down/ls/status/token/url/api/scenarios/logs/exec/install/provision` | Isolated worktree test gateways (**Linux `systemd --user` only** — every systemd-touching verb refuses with a one-line message on macOS/Windows). `api <wt> <METHOD> <path> [--data JSON]` makes ONE authenticated call against a running pod and prints a fixed-key JSON document (`{name, method, path, status, ok, body}`, `body` parsed when it is JSON and raw text otherwise); a non-2xx prints the same document and exits 1. The token is minted through the same ownership-proof path `pod token` uses, so no caller reads `.local_secret` and a foreign process holding the derived port cannot be authenticated against. `scenarios [--json]` lists the seed fixtures `up --seed` accepts, with the one-line `description` from each `fixture.yaml`. See `src/kiro_crew/pod/README.md`. |
| `kirocrew pod up --seed <scenario\|dir>` | Pre-populate the pod's isolated home. A bare NAME is a packaged fixture (`kiro_crew/tests_fixtures/<name>/`, the same registry `gateway --seed` and `testing.fixtures.seeded_home` read) and populates the WHOLE home; a PATH — anything with a separator or a leading `~`/`.` — is a config dir and contributes only its sanitized `config.json`. The name/path split is **syntactic**, so the control plane and the pod's own `boot` cannot disagree without a filesystem lookup; an unknown bare name is refused with the available list, because a seed silently read as a path boots a pod that is blank AND healthy. Both forms record the one `SEED=` key. Seeding runs in `boot` after the HOME exists and before the gateway exec, and a home that already holds state is never re-seeded — a `Restart=on-failure` re-exec must not delete the evidence of the crash. A scenario's own `config.json` is put through `SEED_DISABLED_SECTIONS` (`runtime.sanitize_home_config`), so no fixture can hand a pod a live channel. |
