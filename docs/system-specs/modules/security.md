...
**Fail-closed default when no backend**: when no sandbox backend is available, `wrap_argv()` **raises `RuntimeError`** by default rather than executing the agent unsandboxed — the secure default is to refuse, not degrade. The denial also emits a `denied` SEL tool-invocation event. Running unsandboxed is a deliberate opt-in via `agent.sandbox_allow_unsandboxed_exec=true`. An explicitly classified official Kiro CLI spawn on Windows may delegate for the ordinary default data home already covered by Kiro's internal sandbox. A relocated or linked coordinator ledger has no such proven path contract and fails closed.
...
**macOS sandbox mutual exclusion**: kiro-cli ≥ 2.13 ships an *internal* agent sandbox in the binary itself, toggled by the `"sandbox"` key in `~/.kiro/settings/amazon-internal.json` (the kiro-cli backend's own settings dir — distinct from Kiro Crew's data home `~/.kiro/crew`; the filename is the literal kiro-cli ships). Its in-process seatbelt init cannot nest inside Kiro Crew's sandbox-exec wrap: the macOS kernel returns EPERM even under an `(allow default)` outer profile, so **exactly one sandbox layer can be active per kiro-cli spawn**. The ordinary default coordinator ledger remains inside the data home Kiro already protects and may delegate. A relocated or linked ledger and caller-provided hidden/visible paths are delegation conflicts and fail closed with a typed `foreign_sandbox` error. The relocation check uses normalized lexical paths because it runs on the gateway event loop for every delegated spawn; filesystem identity probes could block on a stalled home mount. A distinct alias therefore fails closed. A process already proven to be inside Kiro Crew's sandbox passes through before these delegation checks because its outer sandbox already hides the canonical ledger and a nested Seatbelt application is impossible. Delegation remains deterministic rather than a reaction to wrap failure, SEL-audited with `outcome="delegated"` and `critical=True`, uses the shared environment scrub, applies only to positively identified kiro-cli spawns, and never changes Linux namespace isolation.

**Windows Kiro internal-sandbox delegation**: Kiro Crew has no native Windows OS wrapper, but the official Kiro CLI backend has its own sandbox. The reviewed caller's `is_kiro_cli=True` classification and an ordinary default coordinator path are both required, so fresh installs retain chat, model, identity and usage functionality without the broad unsandboxed-exec opt-in. Basename inference, relocated or linked ledgers, and caller path overrides remain denied before backend detection. Each delegated spawn is `outcome="delegated"`, `critical=True`, and uses the parent-side `scrub_agent_subprocess_env()` because Windows cannot prefix `env -u`. All other ACP backends, scripts, hooks, third-party MCP probes, Papyrus/Polly commands and future unclassified spawns retain the normal no-backend policy.
...
- Sensitive paths: `~/.aws`, `~/.ssh`, `~/.gnupg`, `~/.gpg`, `~/.config/gcloud`, `~/.azure`, `~/.docker/config.json`, `~/.kube/config`, `~/.npmrc`, `~/.pypirc`, `~/.netrc`, `~/.git-credentials`, `~/.kirocrew.run-coordinator`, `~/.kiro/crew/.env`, `~/.kiro/crew/sel_hmac.key`, `~/.kiro/crew/trust`, `~/.kiro/crew/security_events.jsonl`, `~/.kiro/crew/app_admission.json`, `~/.kiro/crew/workflow_library`, `~/.kiro/crew/run`, `~/.kiro/crew/run-coordinator`
...
- **Run coordinator ledger (`run-coordinator/`)** — the directory contains canonical run, command, lease, and delivery metadata plus SQLite WAL/SHM/journal sidecars. It is on `_CREW_SECRET_LEAVES` as a whole directory so an agent cannot read task payloads, forge terminal delivery, or rewrite its own desired/observed state and fencing metadata. Every Kiro Crew OS-sandbox mode resolves the active data home and denies reads, writes, and hardlinks to its ledger, including when `KIROCREW_HOME` places it outside `Path.home()`; the default and legacy lexical paths remain covered too. An ordinary directory uses its configured coordinator path directly. When the data-home path traverses a link, a canonical directory record under the owner-only `~/.kirocrew.run-coordinator/` anchor directory is keyed by the lexical home selection and shared by SQLite and sandbox construction. The anchor is itself hidden by every Kiro Crew OS sandbox and is on `_SENSITIVE_HOME_DIRS`, so retargeting the link cannot redirect the next gateway process to attacker-prepared state. The macOS Seatbelt profile also denies writes to the active ledger path and each rename-sensitive ancestor as literal targets, so renaming a parent cannot move the ledger outside the path-based subtree rules. The Linux namespace path creates and fail-loud owner-tightens the active ledger directory before building the launcher. It derives the bind mask and rename guards from the canonical ledger spelling, then self-binds every canonical ancestor below the filesystem root as a writable mountpoint (outermost first) before bind-masking the ledger. The mountpoints block ancestor renames without making ordinary app state under the data home read-only. This closes raw subprocess access that bypasses tool-call path matching. The directory, optional anchor record, and known database files are owner-only on creation and reopen.
