...
| [verification-recipes.md](verification-recipes.md) | Verifying a change against a live isolated instance: a deterministic fake-agent pod, driving a conversation over session-control, a WebSocket probe, scale-state loops, soaking the e2e suite, a spec-local accessibility scan, and the boot-duration metric. |
