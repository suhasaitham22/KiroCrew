...
async def api_monitor_create(request: web.Request) -> web.Response:
    ...
    try:
        ...
        if "replace_terminal_id" in body:
            return _monitor_error(
                "terminal monitor replacement is temporarily unavailable",
                "monitor_replace_unsupported",
                status=409,
            )
...
async def api_monitor_restart(request: web.Request) -> web.Response:
    ...
    if loop.monitor.outcome is MonitorOutcome.SESSION_CLOSE:
        return _monitor_error(
